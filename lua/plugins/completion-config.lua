        ...
        providers = {
          lsp = {
            transform_items = function(ctx, items)
              ...
              -- Если query точно совпадает с моим snippet - БЛОКИРУЕМ ВСЁ от LSP!
              if vim.tbl_contains(my_snippets, query) then
                return {} -- Возвращаем пустой список для LSP
              end
          ...
          snippets = {
            transform_items = function(ctx, items)
              ...
              for _, item in ipairs(items) do
                ...
                -- Если МОЙ snippet И точное совпадение - МАКСИМАЛЬНЫЙ boost!
                if is_my_snippet and exact_match then
                  item.score_offset = 99999 -- Безумно большое значение
                end
              end
